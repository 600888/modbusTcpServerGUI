...
class ColoredFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        # Define log color
        LOG_COLORS = {
            'DEBUG': COLOR_CYAN + '%s' + COLOR_RESET,
...
class Log():
    def __init__(self, loggername='', filename=None, mode='a',
                 cmdlevel='DEBUG',
                 filelevel='INFO',
                 cmdfmt='[%(asctime)s] [%(filename)s] [line:%(lineno)d] [%(levelname)s] %(message)s',
                 filefmt='[%(asctime)s] [%(filename)s] [line:%(lineno)d] [%(levelname)s] %(message)s',
    ...
    def set_logger(self, **kwargs):
        ...
        if not "cmdfmt" in kwargs:
            self.filefmt = '[%(asctime)s] [%(filename)s] [line:%(lineno)d] [%(levelname)s] %(message)s'
            self.filedatefmt = '%Y-%m-%d %H:%M:%S'
            self.cmdfmt = '[%(asctime)s] [%(filename)s] [line:%(lineno)d] [%(levelname)s] %(message)s'
            self.cmddatefmt = '%Y-%m-%d %H:%M:%S'
            if self.cmdlevel > 10:
                self.filefmt = '[%(asctime)s] %(levelname)-8s%(message)s'
                self.cmdfmt = '[%(asctime)s] %(levelname)-8s%(message)s'
